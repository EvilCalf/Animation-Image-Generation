...
class SelfAttention(dg.Layer):
    ...
    def forward(self, x):
        ...
        attention = self.softmax(paddle.tensor.bmm(theta, phi))
        g = self.g(x)
        g = layers.reshape(self.pool(g),(m_batchsize, -1, N // 4))
        attn_g = layers.reshape(paddle.tensor.bmm(g, layers.transpose(attention,(0, 2, 1))),(m_batchsize, -1, width, height))
